...
void readPositionSensorsTask(void)  //Read position sensors at 20hz
{
	...
	float volatile accAngle = atan((float)ACCELX_RAW / ACCELZ_RAW) * 180 / M_PI * 1000;
	float volatile gyroAngle = (float)Gyro_Phi_deg * 0.01 * 1000;
	static volatile sint32 filterAngle = 0;
	static volatile sint32 filterAngleOld = 0;

	static sint32 gyroAngleCalc = 0;

	gyroAngleCalc = gyroAngleCalc + gyroAngle;

	filterAngle = (filterAngleOld + gyroAngle) * 0.99 + 0.01 * accAngle;
	filterAngleOld = filterAngle;
	sl_debug_1 = filterAngle;
	sl_debug_2 = accAngle;
	sl_debug_3 = gyroAngleCalc;
	sl_debug_4 = Phi_deg * 100;
